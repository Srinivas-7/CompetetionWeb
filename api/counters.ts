...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // Graceful fallback if Admin DB credentials are not present
    return res.status(200).json({
      success: true,
      counts: instanceCache ? instanceCache.counts : counts,
      totalVotes: instanceCache ? instanceCache.totalVotes : 0,
      updatedAt: new Date().toISOString(),
      fallback: true,
    });
  } catch (err: any) {
    console.error('[Counters Error] Failed to process counters:', err);
