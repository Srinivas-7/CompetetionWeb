...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 1b. Launch Time Gate (15 Sep 2026, 10:00:00 AM IST)
  const LAUNCH_TIMESTAMP = 1789446600000;
  if (Date.now() < LAUNCH_TIMESTAMP) {
    return res.status(403).json({
      success: false,
      error: 'VOTING_NOT_STARTED',
      message: 'Voting has not officially started yet. Bappa is taking a little longer to arrive — thank you for your patience, the celebration begins 15 September 2026 at 10:00 AM IST.',
    });
  }
